#!/usr/bin/env python3
"""
Авторизация новых аккаунтов - использует тот же метод, что и authorize_account.py
(который успешно залогинил Oleg Petrov)
"""
import asyncio
import json
import sys
...
async def authorize_new_account(account_data):
    """Авторизация нового аккаунта - ТОЧНО как в authorize_account.py"""
    ...
    # Создаем клиент - ТОЧНО как в authorize_account.py
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            # Используем тот же метод, что и для Oleg Petrov
...
def main():
    ...
    print("="*80)
    print("Используется тот же метод, что и для Oleg Petrov (@petrsoleg)")
    print("="*80)
    print("\nВыберите аккаунт:")
    print()
    
    for key, account in NEW_ACCOUNTS.items():
        print(f"  {key}. {account['nickname']} ({account['phone']})")
    
    print()
    choice = input("Введите номер аккаунта (1-3): ").strip()
    
    if choice not in NEW_ACCOUNTS:
        print(f"❌ Неверный выбор: {choice}")
        return
    
    account = NEW_ACCOUNTS[choice]
    print(f"\n✅ Выбран: {account['nickname']} ({account['phone']})")
    
    # Запускаем асинхронную функцию
    session = asyncio.run(authorize_new_account(account))
