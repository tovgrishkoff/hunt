#!/usr/bin/env python3
"""
Авторизация нового Telegram аккаунта
Можно использовать интерактивно или с аргументами командной строки
"""
import asyncio
import json
import sys
...
async def authorize_account(phone, api_id, api_hash, session_name, nickname=None):
...
def main():
    parser = argparse.ArgumentParser(description='Авторизация нового Telegram аккаунта')
    parser.add_argument('--phone', type=str, help='Номер телефона (с +)')
    parser.add_argument('--api-id', type=int, help='API ID')
    parser.add_argument('--api-hash', type=str, help='API Hash')
    parser.add_argument('--session-name', type=str, help='Имя сессии (например, promotion_new_user)')
    parser.add_argument('--nickname', type=str, help='Никнейм (необязательно)')
    
    args = parser.parse_args()
    
    # Если аргументы не переданы, запрашиваем интерактивно
    if not all([args.phone, args.api_id, args.api_hash, args.session_name]):
        print("\n" + "="*80)
        print("🔐 АВТОРИЗАЦИЯ НОВОГО TELEGRAM АККАУНТА")
        print("="*80)
        print()
        ...
        nickname = args.nickname or session_name
    
    result = asyncio.run(authorize_account(phone, api_id, api_hash, session_name, nickname))
    if result:
